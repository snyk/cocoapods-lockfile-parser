...
/// e.g. Adjust (4.17.1)
///      Adjust/Core (4.17.1)
export function pkgInfoFromSpecificationString(
  stringRepresentation: string
): PkgInfo {
  const match = stringRepresentation.match(/^((?:\s?[^\s(])+)(?: \((.+)\))?$/);
  if (!match) {
    throw new Error(
      'Invalid string representation for a ' +
...
export function pkgInfoFromDependencyString(
  stringRepresentation: string
): PkgInfo {
  const match = stringRepresentation.match(/^((?:\s?[^\s(])+)(?: \((.+)\))?$/);
  if (!match) {
...
export function rootSpecName(specName: string): string {
